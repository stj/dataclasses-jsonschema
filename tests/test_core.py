...
from dataclasses_jsonschema.type_defs import NULL, JsonDict, Nullable
...
def compose_schema(schema, definitions=None):
    full_schema = {**schema, **{"$schema": "http://json-schema.org/draft-06/schema#"}}
...
def test_field_types():
    ...
    JsonSchemaMixin.register_field_encoders({Currency: DecimalField(precision=2)})
...
def test_literal_types():
    ...
    expected_schema = {
        ...
        "properties": {"bits_per_pixel": {"enum": [8, 16, 24, "true-color", None]}},
    ...
    expected_json_schema = compose_schema(
        {
            ...
            "properties": {"name": {"type": "string"}, "_id": {"type": "integer"}},
            "required": ["_id", "name"],
        }
    )
...
def test_discriminators():
    ...
    expected_dog = {"PetType": "Dog", "name": "Fido", "breed": "Dalmation"}
    ...
    expected_schema = compose_schema(
        {
            ...
            "properties": {"id": {"type": "integer"}, "data": {}},
            "required": ["id", "data"],
        }
    )
